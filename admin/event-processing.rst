...
.. list-table::
   ...
   * - NXSL
     - Built-in driver that executes an NXSL script for each notification, giving
       ...
       * ``$event`` - ``Event`` object that triggered the
         notification. Present only when the notification was generated by an
         event processing policy rule; it is unset for test sends and digest
         messages.

       When the triggering event has a source object, the standard object
       variables (``$node``, ``$object``, etc.) are set up as well.

       The script return value controls the sending result:

       * *null* (no explicit return) - notification sent successfully
       * *false* - permanent failure, no retry
       * positive number - request retry after that number of seconds
       * any other value - notification sent successfully

       If the script fails to compile or aborts with an execution error, the
       channel is marked as failed and the error is written to the server log.
